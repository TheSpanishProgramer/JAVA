package RefactorizandoConJava1;
...
public class VectoresSinRefactorizar {
	...
	public static void main (String args[]) {
		...
		// leer num  =>
		do {
			num = utilidades.teclado.leer_entero("Introduce un número entero mayor que 5: ");			
		} while (num <= 5);
		
		// crear el vector v de num enteros
		int v [] = new int [num];
		
		// leer desde teclado los elementos del vector V 
		for (int i=0; i <v.length; i++) 
			v[i] = utilidades.teclado.leer_entero("Introduce un número entero: ");		
		
		// calcular cuantos números pares e impares hay en el vector
		...
		// Crear el vector de par e impar 
		int par[] = new int [pares];	
		int impar[] = new int [impares];
		
		// pasar los pares e impares a sus respectivos vectores
		impares = pares = 0;
		for (int i=0; i <v.length; i++) {
			if (v[i] % 2 == 0) {
				par [pares] = v[i];
		...
		// mostrar por pantalla los vectores
